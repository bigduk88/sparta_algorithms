...
# 둘째 줄에는 나무의 높이가 주어진다. 나무의 높이의 합은 항상 M보다 크거나 같기 때문에, 상근이는 집에 필요한 나무를 항상 가져갈 수 있다. 높이는 1,000,000,000보다 작거나 같은 양의
# 정수 또는 0이다.
# 출력
...
tree = list(map(int, input().split()))          # tree : 나무 높이를 나무 수 만큼 입력
start, end = 1, max(tree)                       # 이분탐색 검색 범위 설정
while start <= end:                              # 적절한 벌목 높이를 찾는 알고리즘
    mid = (start + end) // 2                    # 자르는 나무 높이
    log = 0                                     # 벌목된 나무 높이 총합
    for i in tree:
        if i > mid:
            log += i - mid                      # log = log + i - mid
    # 벌목 높이를 이분탐색
    if log >= M:                                # 필요한 나무 길이보다 벌목된 나무가 더 많다면
        start = mid + 1
    else:
        end = mid - 1
print(end)